...
const settingsServicePath = path.join(__dirname, '../extension/scripts/services/settings-service.js');
const settingsServiceCode = fs.readFileSync(settingsServicePath, 'utf8');
eval(settingsServiceCode);
...
describe('SettingsService', () => {
  ...
  describe('get', () => {
    ...
    it('should retrieve settings from storage', async () => {
      ...
      expect(chrome.storage.local.get).toHaveBeenCalledWith(['openrouterApiKey', 'aiModel']);
    });
  ...
  describe('removeCached', () => {
    ...
    it('should sort keys before removing', () => {
      const cacheKey1 = 'aiModel,openrouterApiKey';
      const cacheKey2 = 'openrouterApiKey,aiModel';

      service.cache.set(cacheKey1, { value: {}, expiry: Date.now() + 10000 });

      service.removeCached(['openrouterApiKey', 'aiModel']);
      expect(service.cache.has(cacheKey1)).toBe(false);

      service.cache.set(cacheKey2, { value: {}, expiry: Date.now() + 10000 });

      service.removeCached(['aiModel', 'openrouterApiKey']);
      expect(service.cache.has(cacheKey2)).toBe(false);
    });
  });
  ...
  describe('getApiKey', () => {
    ...
    it('should not use cache for encrypted keys check', async () => {
      await chrome.storage.local.set({
        openrouterApiKey: 'test-key',
        encryptionEnabled: false,
      });

      await service.getApiKey('openrouterApiKey');
      await service.getApiKey('openrouterApiKey');

      // Should always fetch fresh to check encryption status
      expect(chrome.storage.local.get).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('chrome.storage.onChanged listener', () => {
    it('should clear cache when storage changes', () => {
      ...
      listener({ openrouterApiKey: { newValue: 'new-key' } }, 'local');

      expect(clearCacheSpy).toHaveBeenCalled();
    });

    it('should not clear cache for non-local storage changes', () => {
      const clearCacheSpy = jest.spyOn(service, 'clearCache');

      const listener = chrome.storage.onChanged.addListener.mock.calls[0][0];
      listener({ openrouterApiKey: { newValue: 'new-key' } }, 'sync');
  ...
  describe('integration tests', () => {
    ...
    it('should handle concurrent get requests efficiently', async () => {
      ...
      // Multiple concurrent gets should use cache
      const promises = [
      ...
      // Should only fetch from storage once
      expect(chrome.storage.local.get).toHaveBeenCalledTimes(1);
    });
